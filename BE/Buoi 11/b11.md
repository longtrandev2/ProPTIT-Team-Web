- [Bảo Mật Mật Khẩu \& Mã Hóa Dữ Liệu: Lý Thuyết Cốt Lõi và Thực Hành (Java)](#bảo-mật-mật-khẩu--mã-hóa-dữ-liệu-lý-thuyết-cốt-lõi-và-thực-hành-java)
  - [1. Mã hóa là gì? (Encryption)](#1-mã-hóa-là-gì-encryption)
  - [2. Các phương pháp Mã hóa chính](#2-các-phương-pháp-mã-hóa-chính)
    - [A. Mã hóa Đối xứng (Symmetric Encryption)](#a-mã-hóa-đối-xứng-symmetric-encryption)
    - [B. Mã hóa Bất đối xứng (Asymmetric Encryption)](#b-mã-hóa-bất-đối-xứng-asymmetric-encryption)
  - [3. Phân biệt Mã hóa (Encryption) và Băm (Hashing)](#3-phân-biệt-mã-hóa-encryption-và-băm-hashing)
  - [4. Kỹ thuật Salting (Thêm "Muối")](#4-kỹ-thuật-salting-thêm-muối)
  - [5. Thực Hành: Code ví dụ (Java Spring Boot)](#5-thực-hành-code-ví-dụ-java-spring-boot)
    - [Trường hợp 1: Lưu mật khẩu an toàn với BCrypt (Hashing)](#trường-hợp-1-lưu-mật-khẩu-an-toàn-với-bcrypt-hashing)
    - [Trường hợp 2: Mã hóa dữ liệu nhạy cảm với AES (Encryption)](#trường-hợp-2-mã-hóa-dữ-liệu-nhạy-cảm-với-aes-encryption)
  - [6. Các thuật toán phổ biến nên biết](#6-các-thuật-toán-phổ-biến-nên-biết)
...
- [CODE MINH HỌA: CSRF PROTECTION TRONG SPRING BOOT](#code-minh-họa-csrf-protection-trong-spring-boot)
    - [1. Kịch bản Lỗ hổng (Khi chưa có CSRF Token)](#1-kịch-bản-lỗ-hổng-khi-chưa-có-csrf-token)
    - [2. Phòng chống bằng CSRF Token (Web truyền thống - Thymeleaf)](#2-phòng-chống-bằng-csrf-token-web-truyền-thống---thymeleaf)
    - [3. Phòng chống cho REST API / Single Page App (React/Angular)](#3-phòng-chống-cho-rest-api--single-page-app-reactangular)
    - [4. Lớp bảo vệ bổ sung: SameSite Cookie](#4-lớp-bảo-vệ-bổ-sung-samesite-cookie)
    - [Tóm lại](#tóm-lại)
